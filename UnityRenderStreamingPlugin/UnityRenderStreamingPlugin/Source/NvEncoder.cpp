...
#pragma region setup resources
			...
			g_D3D11Device->CreateTexture2D(&desc, NULL, &pInputTextures);
			renderTextures[i] = pInputTextures;
#pragma endregion
#pragma region register input buffer
			...
			registerResource.resourceToRegister = (void*)renderTextures[i];
			renderTextures[i]->GetDesc(&desc);
			DebugLog(StringFormat("rederTexture[%d] is %d, format is %d, usage is %d, width is %d, height is %d, bind flag is %d, cpu access flag is %d"
				, i, renderTextures[i], desc.Format, desc.Usage, desc.Width, desc.Height, desc.BindFlags, desc.CPUAccessFlags).c_str());
			...
			result = NV_RESULT((errorCode = pNvEncodeAPI->nvEncRegisterResource(pEncoderInterface, &registerResource)));
			DebugLog(StringFormat("nvEncRegisterResource error is %d", errorCode).c_str());
			checkf(result, "Failed to register input back buffer");
...
#pragma endregion
#pragma region map input buffer resouce
			...
			result = NV_RESULT((errorCode = pNvEncodeAPI->nvEncMapInputResource(pEncoderInterface, &mapInputResource)));
			checkf(result, "Failed to map NVEncode input resource");
			DebugLog(StringFormat("nvEncMapInputResource error is %d", errorCode).c_str());
			frame.inputFrame.mappedResource = mapInputResource.mappedResource;
#pragma endregion
#pragma region create output bitstream buffer
			...
			result = NV_RESULT((errorCode = pNvEncodeAPI->nvEncCreateBitstreamBuffer(pEncoderInterface, &createBitstreamBuffer)));
			DebugLog(StringFormat("nvEncCreateBitstreamBuffer error is %d", errorCode).c_str());
			checkf(result, "Failed to create NVEncode bit stream buffer");
			frame.outputFrame = createBitstreamBuffer.bitstreamBuffer;
#pragma endregion
		}
	}
